...
st.markdown("""
Welcome to my Race Data Analysis Dashboard! This application allows you to upload your own `race_data.csv` file or use our sample data to generate insightful visualizations about typeracer performance.
""")

st.sidebar.header("Race Data")
data_option = st.sidebar.radio("Choose data source:", ["Upload your own data", "Use sample data"])
...
if data_option == "Upload your own data":
    ...
    if uploaded_file is not None:
        ...
        st.info("Please upload a `race_data.csv` file to proceed or select 'Use sample data'.")
        st.stop()
else:
    # Use sample data
    try:
        st.sidebar.info("Using sample race data for demonstration.")
        df = pd.read_csv("race_data.csv")
    except Exception as e:
        st.error(f"Error reading the sample data: {e}")
        st.stop()
